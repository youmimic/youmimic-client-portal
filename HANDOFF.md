...
**`/contact`**: audited `app/(marketing)/contact/page.tsx`, `components/marketing/contact-form.tsx`, `lib/validations/contact.ts`, `app/api/contact/route.ts`. All 5 fields (name/email/company/phone/message) are required — no optional fields exist — so added a plain "All fields are required" note rather than per-field "(required)" tags. Paired Name/Email and Company/Phone into a 2-column grid on `sm:`+ (Message stays full-width) for a tighter, less wall-of-inputs feel. Added `role="alert"` to the error banner and `role="status"` to the success confirmation — both were plain `div`s with no screen-reader announcement despite `FormMessage` itself already having `aria-live="polite"` from an earlier pass. No backend/validation/submission changes — rate limiting, Zod schema, and the Resend email call in `app/api/contact/route.ts` are all untouched.

**Calendly widget — two follow-up user reports, same root cause**: the inline widget was rendering its own internal scrollbar. First attempt: assumed a pure height problem (Calendly's docs specify a 630px minimum) and set `min-h-157.5` (630px) with `h-[min(78vh,760px)]` to scale with viewport up to a 760px cap. User reported the scrollbar persisted below ~2275px viewport width — a threshold that pointed at Calendly's own internal responsive breakpoint (roughly 1000px of *widget* width, not page height): squeezed into one half of an even `lg:grid-cols-2` split, the widget doesn't reach that width until the viewport itself is very wide, so it falls back to a taller internal layout no reasonable fixed height ever satisfies. Fixed by changing the form+Calendly section from `lg:grid-cols-2` to stacking full-width until a custom `min-[2300px]:grid-cols-2` breakpoint — the widget gets the full `lg:w-[90vw]` container's width at every normal desktop/laptop size, and the side-by-side layout only re-appears on monitors wide enough that even a half column clears Calendly's own threshold. Also removed the `border border-border` around the widget per a direct request — kept `rounded-xl overflow-hidden` for the corner-clipping.

**`/login` and `/signup`**: audited both form components plus `components/legal/legal-acceptance-field.tsx`. Same accessibility gap pattern as `/contact` had: the registered/verified/reset success banners (login) and error banners (both) were plain `div`s with no `role="status"`/`role="alert"`; dismiss (X) buttons on all of these had no `focus-visible` treatment. Fixed all of them consistently. Added a password-requirements hint under signup's password field, worded to exactly mirror `lib/validations/auth.ts`'s real `passwordSchema` rules (8–100 characters, upper+lowercase+digit) — nothing invented. Added `focus-visible` and `role="alert"` to `LegalAcceptanceField`'s buttons/error text too, since it's shared with the invite-acceptance flow as well.

**Found, not acted on — flagging per the original prompt's "stop before a behavior-changing decision" instruction**: `registerSchema` defines `accountType: z.enum(["INDIVIDUAL", "BUSINESS"])`, but `signup-form.tsx` hardcodes `accountType: "BUSINESS"` in its `defaultValues` with **no UI control to choose otherwise** — meaning the `INDIVIDUAL` signup path (and its exemption from requiring a business name) is currently unreachable through the actual signup page, even though the validation layer still supports it. Not clear whether this is deliberate (an intentional simplification to business-only self-serve signup) or a UI regression from when a toggle might have existed — left entirely alone rather than guessing either way, since exposing or removing that path is a real behavior change, not a visual one.

**Checks**: `npm run typecheck` clean, `npm run lint` → 0 errors (3 pre-existing warnings, unchanged), `npx vitest run` → 111/111 passing (UI-only, no test changes needed), `npx next build` → clean. Live-checked all three pages return 200, and confirmed via response HTML that the required-fields note, the new Calendly wrapper classes, and the `min-[2300px]:grid-cols-2` breakpoint are all actually present in the rendered markup.

**Not done / next**: remaining milestones — checkout pages (including moving `/dashboard/checkout` out of the dashboard shell to match the guest `/checkout` treatment, per an earlier decision this session) and each dashboard sub-page (dashboard home, avatars, videos, billing, billing/payments, bookings, settings).

### Follow-up same session: decorative gradient/blur request, then a partial revert

User asked to add blur/gradient effects to `/login`, `/signup`, and `/contact` — noted the tension with the original scoping prompt's own "avoid excessive gradients/glassmorphism/blur" instruction, then applied a restrained version consistent with the brand's existing subtle-radial-gradient precedent (already used in `/contact`'s hero and form sections): a `blur-3xl`, low-opacity (`bg-accent/20`/`bg-primary/10`) decorative blob positioned behind an otherwise fully opaque `Card` on each page — never touching the card surface itself, so contrast/readability is unaffected. Also added `min-h-screen` to `/login` and `/signup`'s `<main>` after the user reported the card "wasn't centered" — the actual cause: `flex-1` alone only centers within whatever short strip is left between the marketing header and the (fairly tall) marketing footer, not the real viewport height.

User then asked to revert both changes on `/login` specifically ("looks too bad") — reverted the decorative blobs and the `min-h-screen` centering fix there, back to the exact original `<main className="container mx-auto flex flex-1 max-w-lg items-center justify-center px-4 py-10">`. The earlier Milestone 2 accessibility fixes (banner `role`s, `focus-visible` states) were untouched by either the addition or the revert. `/signup` and `/contact` keep both the decorative blur/gradient accents and (for `/signup`) the `min-h-screen` centering fix, since only `/login` was called out.

**Checks after the revert**: `npm run typecheck` clean, `npm run lint` → 0 errors (3 pre-existing warnings, unchanged), `npx vitest run` → 111/111 passing, `npx next build` → clean.

### Follow-up same session: root-caused and fixed the login/signup-to-footer gap properly

User asked why there was a large visible gap between the login page's content and the marketing footer. Root cause: `<main>` on both `/login` and `/signup` used `flex-1 items-center justify-center` inside the root layout's `flex flex-col` body — that stretches `main` to fill *all* leftover vertical space between the header and the (fairly tall) marketing footer, then centers the short auth card within that entire stretched region. On any screen taller than header+card+footer combined (most screens), that leaves large, variable-sized empty space both above and below the card — the "below" half is what shows up as a big gap right before the footer. This was also the same underlying mechanism the earlier `min-h-screen` change made worse (guaranteeing an even taller region to center within) before it was reverted on `/login`.

**Fix, applied to both `/login` and `/signup`** for consistency (signup still had the more exaggerated `min-h-screen` version of the same issue): replaced `flex flex-1 items-center justify-center` with fixed vertical padding (`py-16 sm:py-24`) — `main` now sizes to its actual content plus a consistent, predictable gap, rather than stretching to fill whatever space happens to be left before the footer. `/signup` keeps its `relative overflow-hidden` (needed for its decorative gradient blobs, untouched by this fix) and its blur/gradient accents; `/login` remains exactly as it was before any of today's decorative or centering changes.

**Bug introduced and fixed while making this change**: first attempt wrote the explanatory comment as a JSX `{/* ... */}` block placed directly between `return (` and the opening `<main>` tag — invalid, since a bare `{/* */}` can only appear as a child *within* an element, not floating before the JSX root itself. This produced real parse errors (`tsc`: `')' expected`; `eslint`: `Parsing error`) on both files. Fixed by converting both to plain `//` line comments placed above `return (`, matching how comments already precede JSX elsewhere in this codebase.

**Checks**: `npm run typecheck` clean, `npm run lint` → 0 errors (3 pre-existing warnings, unchanged), `npx vitest run` → 111/111 passing, `npx next build` → clean. Live-checked both `/login` and `/signup` return 200 and confirmed via response HTML that `flex-1 items-center` is gone and `py-16 sm:py-24` is present.

## Session: UI/UX pass, Milestone 1 — dashboard shell/navigation — 2026-09-13

User handed over a large, carefully-scoped UI/UX/frontend prompt (principal-designer framing, explicit anti-fabrication guardrails, phased structure) limited to `/contact`, all `/dashboard` pages, and the checkout pages — explicitly excluding the landing page, marketing pages, and pricing page. Given the size, agreed on two decisions before starting: (1) the authenticated `/dashboard/checkout` page will get the same minimal-header/two-column treatment as the guest `/checkout` page rather than staying inside the dashboard shell — a later milestone; (2) work proceeds one milestone at a time rather than as one continuous pass, matching how the rest of this session's features were built.

**This milestone: the dashboard shell** (`components/dashboard/dashboard-shell.tsx`, `dashboard-header.tsx`, `app-sidebar.tsx`, `app/(dashboard)/layout.tsx`) — the highest-leverage piece, since it wraps every dashboard page at once.

**Audit finding that shaped scope**: every dashboard page already renders its own consistent `<h1>` (confirmed via grep across all 6 top-level pages) — so adding a page title to the shared header, as the original prompt's own checklist suggested, would have just duplicated what's already there. Skipped that; focused instead on the shell's actual gaps:

- **Mobile sidebar had none of the keyboard/focus handling** that `components/marketing/marketing-nav.tsx`'s mobile panel already got in the 2026-09-07 UI/UX pass — no Escape-to-close, no body-scroll-lock, no `role="dialog"`/`aria-modal`, no `aria-expanded`/`aria-controls` on the toggle button, no close-on-route-change. Fixed by mirroring that exact established pattern: the "adjust state during render" close-on-pathname-change trick (avoids the `react-hooks/set-state-in-effect` lint rule), and a `useEffect` locking body scroll + listening for Escape while open. This logic was lifted into `DashboardShell` (which owns the `mobileOpen` state) rather than `AppSidebar`, since the toggle button itself lives in the separate `DashboardHeader` component — a `toggleButtonRef` is created in the shell and threaded to the header so Escape can return focus to it.
- **No focus-visible states** on the hand-rolled sidebar nav `<Link>`s and the mobile close button (the shared `Button` component already has this baked in; these don't use it). Added the same `focus-visible:ring-3 focus-visible:ring-ring/50` treatment used elsewhere.
- **No workspace/company identity anywhere in the shell** — for the real multi-user `Enterprise` accounts this app has, there was no visible indication of which company you're in. Added a small identity block (company name + role) at the top of the sidebar, fetched via one extra `enterpriseMember.findFirst` query in the dashboard layout — but deliberately **only rendered when a membership row actually exists**. Individual Creator/Mid Market/Small Business accounts have no workspace concept at all, so nothing is shown for them rather than inventing one (a distinction surfaced and confirmed earlier in this same conversation, during the Adam Spencer/Concinnity billing investigation).
- Added `aria-current="page"` to the active nav link.

**Bug found and fixed along the way**: passing a `ref` prop to the shared `Button` component (needed for the toggle-button/Escape-refocus wiring) failed typecheck — `ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>`, which doesn't include `ref` in its type even though React 19 supports ref as a plain prop at runtime. Fixed `components/ui/button.tsx` itself (added `ref?: React.Ref<HTMLButtonElement>` to the interface, destructured it, and forwarded it to both the `asChild`/`Slot` branch and the plain `ButtonPrimitive` branch) — a real gap in a shared primitive, not code specific to this feature, so it's now fixed for every future caller that needs a Button ref, not just this one.

**Checks**: `npm run typecheck` clean, `npm run lint` → 0 errors (3 pre-existing warnings, unchanged), `npx vitest run` → 111/111 passing (no test changes needed — this is UI-only), `npx next build` → clean. Live-checked that `/dashboard` still correctly redirects an unauthenticated visitor to `/login`. Not verified via an actual logged-in browser session (no Playwright in this project, and no test credentials on hand to script a real login) — recommend a manual click-through to confirm the workspace-identity block renders correctly for an Enterprise account and the mobile sidebar's Escape/focus behavior feels right before calling this milestone fully done.

**Not done / next**: individual dashboard sub-pages (avatars, videos, billing, billing/payments, bookings, settings, dashboard home), `/contact`, and both checkout pages (including the agreed dashboard/checkout → guest-checkout-style treatment) are all still pending as separate milestones.
...
fix earlier today — user asked to investigate the `next`/`postcss`/`sharp`
findings (8 high-severity, several moderate) next.

**Investigated exploitability against this app's actual config before
upgrading**, same approach as the next-auth fix:
- Several `next` CVEs are Server-Actions-related (DoS, SSRF, unbounded
  payload, endpoint disclosure). Grepped for `"use server"` — found exactly
  one: `app/(auth)/login/actions.ts`'s `loginUser`. Checked its usage —
  it's **dead code**, never imported anywhere (the real login form calls
  `signIn()` client-side directly instead), so that specific action isn't
  even reachable.
- The middleware/proxy bypass CVE requires Turbopack + single-locale i18n
  routing — this app has no i18n config at all (grepped `next.config.ts`),
  so not applicable.
- The SSRF-via-rewrites CVE requires a `rewrites()` config with an
  attacker-influenced destination — this app defines none directly (only
  Sentry's `tunnelRoute` injects one, to a fixed Sentry ingest endpoint,
  not user input).
- The SVG Image-Optimization DoS requires `images.dangerouslyAllowSVG` —
  not set, so not applicable.
- Despite low exploitability for most individual findings, the fix is a
  routine semver-minor bump (`next` 16.2.7 → 16.3.1, `fixAvailable:
  {isSemVerMajor: false}` per `npm audit`), so upgraded anyway rather than
  leaving known CVEs unpatched on reasoning alone. Checked the 16.2.11
  security-patch release notes (the exact boundary all these CVEs are
  fixed at) before upgrading — no breaking changes documented there.

**`postcss` needed a second step** — `next@16.3.1` alone didn't clear it,
because this repo's `package.json` has an `overrides` entry pinning
`postcss: "^8.5.15"`, which resolved to exactly `8.5.15` (vulnerable; fixed
at `8.5.23`+). No documented reason found for that specific pin (not
referenced anywhere in `HANDOFF.md`'s history), so bumped the override to
`^8.5.26` (latest).

**Changes:**
- `package.json` — `next` `16.2.7` → `16.3.1`, `eslint-config-next`
  `16.2.7` → `16.3.1` (kept in lockstep, matching the existing exact-pin
  convention), `overrides.postcss` `^8.5.15` → `^8.5.26`.
- `AGENTS.md` — auto-regenerated by the newer `next dev`'s own tooling
  (`node_modules/next/dist/server/lib/generate-agent-files.js`) when I ran
  it during verification; not a manual edit. Confirmed via the file's own
  new text: "This block is written and re-added by `next dev`... committing
  it with your work keeps the tree clean." Harmless, expected side effect
  of the version bump.

## Verification

```
npm install         → next/sharp/postcss all cleared from npm audit;
                       13 vulnerabilities remain (1 low, 8 moderate, 4
                       high — all unrelated packages, not investigated
                       this session)
npm run lint         → 0 errors, 2 pre-existing warnings (unchanged)
npm run typecheck    → clean
npm test              → 30/30 passing, unchanged
...
This upgrade touches routing/middleware compilation, the CSS build
pipeline (postcss), and Turbopack dev tooling — a bigger surface than the
next-auth bump — so verification went beyond the automated checks. Ran a
real dev server and confirmed: all 6 security headers still present
(including CSP/HSTS added earlier today), `/robots.txt` and `/sitemap.xml`
still 200, Tailwind CSS still compiles and is linked in the HTML (confirms
the postcss override bump didn't break the build pipeline), `/dashboard`
still 307-redirects to `/login` for unauthenticated requests (proxy.ts
middleware intact), the branded 404 page still renders, and a real
CSRF-carrying login attempt still returns the identical
`error=CredentialsSignin&code=invalid_credentials` response as before the
upgrade. Dev server log showed no new errors. Cleaned up test
`rate_limit_buckets` rows afterward.

## Not done / next

- The remaining 13 `npm audit` findings (js-yaml, valibot, brace-expansion,
  fast-uri, ip-address, @opentelemetry/*, hono, body-parser — all
  transitive deps of `shadcn`, `@sentry/nextjs`'s toolchain, `prisma`, or
  `eslint`, not runtime dependencies of the app itself) were not
  investigated this session.
- Still open: console.* → pino cleanup, broader test coverage, CSP
  nonce-based script-src, `(dashboard)`/`(admin)`-specific error/not-found
  boundaries.
...
**One real ambiguity surfaced, not guessed past**: this app never registers a persistent webhook endpoint (`POST /v3/webhooks/endpoints`) — it passes a one-off `callback_url` per video-generation request instead (HeyGen's alternative, no-endpoint-needed mechanism, per the same docs). The "Verifying Payloads" section is written entirely in terms of the registered-endpoint flow (a `secret` returned by `POST /v3/webhooks/endpoints` or the rotate-secret call) and never explicitly states whether one-off `callback_url` deliveries carry the same `Heygen-Signature`/`Heygen-Event-Id` headers, or are signed at all. Resolved below rather than left open.

### Addendum, same day — registered the real webhook endpoint, confirmed working in production

Once the app was actually deployed (see the Vercel cron-schedule fix above — that was the blocker keeping every commit from deploying at all), the ambiguity above became moot: rather than resolve whether ad-hoc `callback_url` deliveries are signed, registered a real persistent endpoint instead — the one path HeyGen's docs fully specify.

- Confirmed via `GET /v3/webhooks/endpoints` that zero endpoints existed on the account before this.
- Registered one for real: `POST /v3/webhooks/endpoints` with the production URL (`https://youmimic-client-portal.vercel.app/api/webhooks/heygen`) and `events: ["avatar_video.success", "avatar_video.fail"]`. Response `201`, `status: "enabled"`. The one-time `secret` was captured and written directly into local `.env` without ever appearing in the conversation — same handling as every other secret this session.
- The user added the same secret to Vercel's Environment Variables (local `.env` alone doesn't reach the deployed app) and redeployed.
- **Verified the entire pipeline against the real production URL, not just locally**: before the Vercel env var was added, `POST https://youmimic-client-portal.vercel.app/api/webhooks/heygen` correctly returned the app's own `500 "Webhook secret not configured"`. After the user added it, that error disappeared; a hand-signed request with a deliberately wrong signature correctly returned `401 "Invalid signature"`, and a correctly-signed request (real secret, real HMAC) returned `200 {"received":true}`. Full round-trip confirmed working in production.

### Not done / next

- **The connected HeyGen account has no video-generation credits right now** — confirmed directly against the live API, not assumed. Nothing will actually render until credits are purchased in the HeyGen dashboard; the webhook and generation pipeline are otherwise fully verified end-to-end, including in production.
...
- The per-request `callback_url` is still also passed on every generation (belt-and-suspenders alongside the new registered endpoint) — harmless either way since the handler is idempotent and dedups by event id, but worth a decision later on whether to keep both delivery paths or simplify to just the registered endpoint.
