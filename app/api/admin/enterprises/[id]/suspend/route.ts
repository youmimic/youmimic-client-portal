...
import { suspendEnterpriseSchema } from "@/lib/validations/admin";
...
export async function POST(
  ...
  const parsed = suspendEnterpriseSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  if (target.status === "suspended") {
    return NextResponse.json({ error: "Enterprise is already suspended" }, { status: 409 });
  }
